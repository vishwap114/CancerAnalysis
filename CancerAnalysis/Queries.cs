namespace CancerAnalysis
{
    public class Queries
    {
        ...
        public static string tupleCount = "SELECT COUNT(*) AS tupleCount FROM Mvdoshi.Patient";
        ...
        public static string PatientCountByCancerType = "SELECT s.site_name, COUNT(c.Patient_ID) AS NoOfPatients " +
                                        "FROM MVDOSHI.Site_Master s " +
                                        "JOIN PSelugar.CASE c ON c.site = s.Site_ID " +
        ...
        public static string YearPatientQuery = "SELECT COUNT(c.patient_id) as PatientCount, s.site_name, Year_Of_Diagnosis " +
                                        "FROM PSelugar.Case c " +
                                        "JOIN MVDOSHI.Site_Master S ON S.Site_ID = c.Site " +
                                        "WHERE s.site_name = $CANCER_TYPE$" +
                                        " GROUP BY c.Year_Of_Diagnosis, s.site_name";
        ...
        public static string SurvivalRateQuery = "SELECT CancerType1,ROUND(AliveCount/TotalCount*100, 2) AS PercentageOfAlive " +
                                        "FROM " +
